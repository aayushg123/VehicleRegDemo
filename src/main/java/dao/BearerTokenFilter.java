...
public class BearerTokenFilter implements ContainerRequestFilter {
	...
	private static final String SECURED_URL_PREFIX = "vehiclereg";
			...
			if (authHeader != null && authHeader.size() > 0) {
				...
				try {
					...
					if (authData != null && !authData.isEmpty()) {
						System.out.println("Authorisation is working");
						return;
					} else {
				...
				System.out.println("Authorisation is not working");
				addReponseToJson(json, Status.ERROR.name(), Response.Status.UNAUTHORIZED.getStatusCode(), "Authorization Header is empty.");
				Response unauthorizedStatus = Response.status(Response.Status.UNAUTHORIZED).entity(json.toString()).build();
				requestContext.abortWith(unauthorizedStatus);
			}
		}
		System.out.println("Authorisation is not working due to the failing if statement!!");
	}
